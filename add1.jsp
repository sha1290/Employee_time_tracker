...
<script>
  // Function to generate a random task ID (numbers only)
  function generateTaskId() {
    return Math.floor(Math.random() * 10000); // Generates a number between 0 and 9999
  }

  // Function to check if generated task ID already exists in the database
  function isTaskIdUnique(taskId) {
    // Replace with your logic to check uniqueness (e.g., Ajax call to server)
    // For demonstration purposes, assume always unique in this example
    return true;
  }

  // Automatically generate task ID when the page loads
  ...
  // Validate date input before form submission
  document.getElementById('taskForm').addEventListener('submit', function(event) {
    ...
    // Proceed with form submission if date is valid (actual submission logic should be handled in the servlet)
    this.submit();
  });
